...
class Teacher extends Base
{
    ...
    public function index(){
        ...
        $list=Userteacher::where($map)->order('id  asc')->paginate(10,false,$pageParam);
        $page=$list->render();
    ...
    public function detail(){
        if(request()->isGet()){
            $data=input();
            $data['status']=['=','1'];
            $res=Userteacher::where($data)->field('id,username,penname,sex,num,pic,phone,email,create_time,update_time,loginIP,login_time')->find();
            if($res){
    ...
    public function edit(){
        ...
        $res=Userteacher::where($map)->field('id')->find();
        if($res){
    ...
    public function password(){
        ...
        if(request()->isPost()){
            $data=input('post.');
            $status['status']=0;
            $result=$this->validate($data,'Userteacher.personalPassword');
             if(true !== $result){
                // 验证失败 输出错误信息
                $status['msg']=$result;
                return json($status);
