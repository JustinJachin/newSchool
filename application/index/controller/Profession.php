...
use app\index\model\Profession as ProfessionModel;
use app\index\model\Academey;
use think\Db;
/**
 * 专业管理
 */
class Profession extends Base
{
    ...
    public function index(){
        ...
        if($keyword){
            ...
            $mapP=['academey_name'=>['like',"%{$keyword}%"]];
            $res=Academey::where($mapP)->column('id');
            //查询条件
            $map=array(
                'academey_id'=>['in',implode(',',$res)],
                'profession_name'=>['like',"%{$keyword}%"],
            );
            $status=['status'=>['=','1']];
            $list=ProfessionModel::whereOr($map)->where($status)->with('academey')->order('id  asc')->paginate(10,false,$pageParam);
        }else{
            //查询条件
            $status=['status'=>['=','1']];
            $list=ProfessionModel::where($status)->with('academey')->order('id  asc')->paginate(10,false,$pageParam);
        }
